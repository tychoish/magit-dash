;;; magit-dash-submodules.el --- Recursive submodule bump for magit-dash -*- lexical-binding: t -*-
...
;;; Commentary:
;; Provides `magit-dash-bump-submodules-menu' for committing updated submodule
...
(declare-function magit-dash--run-git "magit-dash")
(declare-function magit-dash--repo-at-point "magit-dash")
(declare-function magit-dash--maybe-refresh "magit-dash")
...
(defun magit-dash-bump--parse-dirty-submodule-lines (lines)
...
(defun magit-dash-bump--parse-all-submodule-relpaths (lines)
...
(defun magit-dash-bump--parse-porcelain-tracked (lines)
...
(defun magit-dash-bump--all-changes-are-submodules-p (tracked-paths dirty-relpaths)
...
Calls `git submodule status' and returns only entries prefixed with `+'."
  (magit-dash-bump--parse-dirty-submodule-lines
   (magit-dash-bump--git-lines path "submodule" "status")))

(defun magit-dash-bump--all-submodule-relpaths (path)
  "Return all initialized submodule relative paths in repo at PATH."
  (magit-dash-bump--parse-all-submodule-relpaths
   (magit-dash-bump--git-lines path "submodule" "status")))

(defun magit-dash-bump--only-submodule-changes-p (path dirty-relpaths)
  ...
  (magit-dash-bump--all-changes-are-submodules-p
   (magit-dash-bump--parse-porcelain-tracked
    (magit-dash-bump--git-lines path "status" "--porcelain"))
   dirty-relpaths))

;;;; Tree discovery

(defun magit-dash-bump--parent-repo (path)
...
The returned path never has a trailing slash."
  (let* ((normalized (directory-file-name (expand-file-name path)))
    ...
    (when (and parent-dir
               ...
               (magit-dash-bump--repo-root-p parent-dir))
      (let ((rel (file-relative-name normalized parent-dir)))
        (when (seq-some (lambda (sp) (equal rel sp))
                        (magit-dash-bump--all-submodule-relpaths parent-dir))
          (directory-file-name parent-dir))))))

(defun magit-dash-bump--topmost-parent (path)
...
Returns PATH itself when it is not a submodule of any parent."
  (let ((current (expand-file-name path))
    ...
    (while (when-let* ((p (magit-dash-bump--parent-repo current)))
             (setq top p current p)))
    top))

(defun magit-dash-bump--walk-tree (root)
     ...
     (lambda (rel-path)
       (let ((abs-path (expand-file-name rel-path root)))
         (when (and (file-directory-p abs-path)
                    (magit-dash-bump--repo-root-p abs-path))
           (setq result (append result (magit-dash-bump--walk-tree abs-path))))))
     (magit-dash-bump--all-submodule-relpaths root))
    result))

;;;; Utility

(defun magit-dash-bump--path-depth (path)
  "Return the number of directory components in PATH."
  (length (seq-remove #'string-empty-p
                      (split-string (expand-file-name path) "/"))))

(defun magit-dash-bump--make-bump-message (dirty-pairs)
...
(defun magit-dash-bump--args-flag (args flag)
  "Return t when FLAG is present in the transient ARGS list."
  (and (member flag args) t))

;;;; Parallel fetch / pull

(defun magit-dash-bump--fetch-pull-parallel (paths do-fetch do-pull on-all-done)
           ...
           (let ((name (file-name-nondirectory (directory-file-name path)))
                 (finish (lambda (status &optional err)
                           (when (eq status 'error)
                             (message "magit-dash-bump: fetch/pull error in %s: %s"
             ...
             (if do-pull
                 (progn
                   (message "magit-dash-bump: pulling %s..." name)
                   (magit-dash--run-git
                    path '("pull" "--rebase")
                    (lambda (_) (funcall finish 'ok))
                    (lambda (out code)
                      (funcall finish 'error (format "exit %d: %s" code out)))))
               (message "magit-dash-bump: fetching %s..." name)
               (magit-dash--run-git
                path '("fetch" "--all")
                (lambda (_) (funcall finish 'ok))
                (lambda (out code)
...
(defun magit-dash-bump--process-one-repo (path push dry-run on-complete)
...
`error'."
  (let* ((name (file-name-nondirectory (directory-file-name path)))
         (dirty (magit-dash-bump--dirty-submodule-pairs path)))
    (cond
     ((null dirty)
      (message "magit-dash-bump: %s — no dirty submodules, skipping" name)
      (funcall on-complete 'skipped))
     ((not (magit-dash-bump--only-submodule-changes-p path (seq-map #'car dirty)))
      (message "magit-dash-bump: %s — non-submodule changes present, skipping" name)
      (funcall on-complete 'skipped))
     (dry-run
      (message "magit-dash-bump: [dry-run] %s — would commit: %s"
               name (magit-dash-bump--make-bump-message dirty))
      (funcall on-complete 'ok))
     (t
      (let ((msg (magit-dash-bump--make-bump-message dirty))
            (rel-paths (seq-map #'car dirty)))
        (message "magit-dash-bump: %s — staging %s"
                 name (string-join rel-paths " "))
        (magit-dash--run-git
         path (cons "add" rel-paths)
         (lambda (_)
           (message "magit-dash-bump: %s — committing: %s" name msg)
           (magit-dash--run-git
            path (list "commit" "-m" msg)
            (lambda (_)
              (message "magit-dash-bump: %s — committed" name)
              (if (not push)
                  (funcall on-complete 'ok)
                (message "magit-dash-bump: %s — pushing..." name)
                (magit-dash--run-git
                 path '("push")
                 (lambda (_)
                   (message "magit-dash-bump: %s — pushed" name)
                   (funcall on-complete 'ok))
                 (lambda (out code)
                   (message "magit-dash-bump: %s — push failed (exit %d): %s"
                            name code out)
                   (funcall on-complete 'error
                            (format "push failed (exit %d): %s" code out))))))
            (lambda (out code)
              (message "magit-dash-bump: %s — commit failed (exit %d): %s" name code out)
              (funcall on-complete 'error
                       (format "commit failed (exit %d): %s" code out)))))
         (lambda (out code)
           (message "magit-dash-bump: %s — add failed (exit %d): %s" name code out)
           (funcall on-complete 'error
                    (format "add failed (exit %d): %s" code out)))))))))

;;;; Sequential processing loop

(defun magit-dash-bump--process-remaining (remaining results push dry-run on-done)
...
RESULTS accumulates (name . status) pairs.  Calls ON-DONE with the final alist."
  (if (null remaining)
      (let ((ok (seq-count (lambda (r) (eq 'ok (cdr r))) results))
        ...
        (message "magit-dash-bump: complete — %d committed, %d skipped%s"
                 ok skipped
                 (if errors
                     (format ", %d errors (%s)"
                             (length errors)
    ...
    (let* ((path (car remaining))
      ...
      (magit-dash-bump--process-one-repo
       path push dry-run
       (lambda (status &optional _err)
         (magit-dash-bump--process-remaining
          (cdr remaining)
...
(cl-defun magit-dash-bump-submodules-run (path &key fetch pull push dry-run recursive on-done)
...
Keyword arguments:
  ...
  (let* ((root (if recursive (magit-dash-bump--topmost-parent path) path))
         (root-name (file-name-nondirectory (directory-file-name root)))
         (all-paths (magit-dash-bump--walk-tree root))
         (sorted-paths (seq-sort
                        (lambda (a b)
                          (> (magit-dash-bump--path-depth a)
                             (magit-dash-bump--path-depth b)))
                        all-paths)))
    (message "magit-dash-bump: scanning %d repo(s) from %s%s"
             (length sorted-paths) root-name
             (if dry-run " [dry-run]" ""))
    (magit-dash-bump--fetch-pull-parallel
...
(defun magit-dash-bump-submodules-at-point ()
  "Bump submodule pointers for the repository at point in the dashboard."
  (interactive)
  (let* ((args (transient-args 'magit-dash-bump-submodules-menu))
         (path (magit-dash-repo-path (magit-dash--repo-at-point))))
    (magit-dash-bump-submodules-run
     path
     :fetch (magit-dash-bump--args-flag args "--fetch")
     :pull (magit-dash-bump--args-flag args "--pull")
     :push (magit-dash-bump--args-flag args "--push")
     :dry-run (magit-dash-bump--args-flag args "--dry-run")
     :recursive (magit-dash-bump--args-flag args "--recursive"))))

(defun magit-dash-bump-submodules-all ()
  ...
  (let* ((args (transient-args 'magit-dash-bump-submodules-menu))
         ...
         (roots (delete-dups
                 (seq-map
                  (lambda (repo)
                    (let ((rpath (magit-dash-repo-path repo)))
                      (if recursive (magit-dash-bump--topmost-parent rpath) rpath)))
                  magit-dash-repo-list))))
    (unless roots
      (user-error "No repositories registered in magit-dash-repo-list"))
    (let* ((all-paths (delete-dups (seq-mapcat #'magit-dash-bump--walk-tree roots)))
           (sorted-paths (seq-sort
                          (lambda (a b)
                            (> (magit-dash-bump--path-depth a)
                               (magit-dash-bump--path-depth b)))
                          all-paths)))
      (message "magit-dash-bump: scanning %d repo(s) across all registered roots%s"
...
(transient-define-prefix magit-dash-bump-submodules-menu ()
...
children are committed and pushed before their parents record the new hashes."
  ...
  [["Options"
   ...
   ["Actions"
    ("b" "Bump repo at point" magit-dash-bump-submodules-at-point
     :inapt-if-not (lambda ()
                     (and (derived-mode-p 'magit-dash-mode)
                          (tabulated-list-get-id))))
    ("B" "Bump all registered repos" magit-dash-bump-submodules-all)]])

(provide 'magit-dash-submodules)
;;; magit-dash-submodules.el ends here
